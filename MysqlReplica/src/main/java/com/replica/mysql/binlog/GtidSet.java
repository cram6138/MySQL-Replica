...
public class GtidSet {
	...
	private final Map<String, UUIDSet> map = new LinkedHashMap<String, GtidSet.UUIDSet>();

	/**
	 * @param gtidSet gtid set comprised of closed intervals (like MySQL's
	 *                executed_gtid_set).
	 */
	...
	/**
	 * Get an immutable collection of the {@link UUIDSet range of GTIDs for a single
	 * server}.
	 * 
	 ...
	 * @return the {@link UUIDSet} for the identified server, or {@code null} if
	 *         there are no GTIDs from that server.
	 */
	 ...
	 * @return the old {@link UUIDSet} for the server given in uuidSet param, or
	 *         {@code null} if there are no UUIDSet for the given server.
	 */
	...
	/**
	 * Determine if the GTIDs represented by this object are contained completely
	 * within the supplied set of GTIDs. Note that if two {@link GtidSet}s are
	 * equal, then they both are subsets of the other.
	 * 
	 * @param other the other set of GTIDs; may be null
	 * @return {@code true} if all of the GTIDs in this set are equal to or
	 *         completely contained within the supplied set of GTIDs, or
	...
	public static final class UUIDSet {
		...
		/**
		 * Determine if the set of transaction numbers from this server is completely
		 * within the set of transaction numbers from the set of transaction numbers in
		 * the supplied set.
		 * 
		 * @param other the set to compare with this set
		 * @return {@code true} if this server's transaction numbers are equal to or a
		 *         subset of the transaction numbers of the supplied set, or false
		 *         otherwise
		 */
	...
	public static final class Interval implements Comparable<Interval> {
		 ...
		 * @return {@code true} if the {@link #getStart() start} is greater than or
		 *         equal to the supplied interval's {@link #getStart() start} and the
		 *         {@link #getEnd() end} is less than or equal to the supplied
